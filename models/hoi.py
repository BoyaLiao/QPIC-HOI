...
class PostProcessHOI(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        print(out_sub_boxes.shape, out_obj_boxes.shape, out_verb_logits.shape,  out_obj_logits.shape)
        assert len(out_obj_logits) == len(target_sizes)
        assert target_sizes.shape[1] == 2

        obj_prob = F.softmax(out_obj_logits, -1)
        obj_scores, obj_labels = obj_prob[..., :-1].max(-1) #最大值，最大值处的索引
        ...
        for os, ol, vs, sb, ob in zip(obj_scores, obj_labels, verb_scores, sub_boxes, obj_boxes):
            ...
            ids = torch.arange(b.shape[0]) # 0, 1, ..., 2n-1
